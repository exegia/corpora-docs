...
.PHONY: help ci validate-docs pr-guard \
...
ci: validate-docs ## Everything CI runs on a pull request.

validate-docs: ## Check docs.json parses and every page it lists exists.
	@python3 $(SCRIPTS)/validate_docs.py

# There is no `validate-sections` target. The sections registry is checked by
# reconcile-sections.yml, which validates it against the mirror schema and also
# re-runs the mirror — a network check that has no place in an offline `ci`.
# The submodule-era validator that used to sit here was tied to the nav-merge
# code that mirroring replaced.
