...
jobs:
  dispatch:
    ...
    steps:
      ...
      - name: Warn when the token could not be minted
        if: steps.app-token.outputs.token == ''
        run: |
          echo "::warning::Could not mint a token for exegia/corpora-docs. Check that the \
          automation App is installed there with contents:write. The docs site will pick \
          this change up on its nightly mirror instead."
```

### Authentication

The dispatch authenticates with a **short-lived token minted from the org automation App**, not a personal access token. `AUTOMATION_APP_ID` and `AUTOMATION_APP_PRIVATE_KEY` are org secrets with `visibility=all`, so every exegia repo can already read them — a new source repo needs **no secret provisioned at all**.

`owner` + `repositories` scope the token to `corpora-docs` alone, and it expires within the hour. Nothing long-lived exists for this, so there is nothing to rotate.

The App must have access to `exegia/corpora-docs` with `contents: write`, which is what the `dispatches` endpoint requires.

<Note>
`continue-on-error` on the minting step is not optional boilerplate. If the App lacks access to `corpora-docs`, minting fails — and without it that would put a red X on **every** push to `docs/`, which reads as "this repo's CI is broken" rather than "one optimisation isn't wired up yet". Warning instead means the workflow can merge before access is granted and starts working the moment it is. The nightly mirror covers the gap either way.
</Note>
