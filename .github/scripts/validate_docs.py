...
Pages under `content/<slug>/` are mirrored into this repository by
scripts/mirror_sections.py, so they are ordinary committed files and are held
to exactly the same standard as the hub's own pages. An earlier version skipped
them when the directory was empty, which made sense while sections were git
submodules; under mirroring an empty `content/<slug>/` means the mirror has not
run or a slug was archived without its navigation being cleaned up. That is the
precise failure PR #12 found — navigation renders, every page 404s, and the
build reports nothing — so it must be red, not skipped.
"""
...
def iter_pages(node, in_pages: bool = False):
    ...
    The top level is keyed `groups` (or `tabs`, `anchors`, `versions`,
    `languages`, depending on the shape Mintlify is given), so the walk cannot
    assume a fixed depth. A path string counts as a page only when it is
    reached through a `pages` list; every other key is just a container to
    descend through, which keeps `"openapi": "spec.json"` and friends from
    being mistaken for pages.
    """
...
def main() -> int:
    ...
    print(f"docs.json: {checked} page(s) validated")
    return 0
